...
export default function MyHotels() {
    ...
    const hotelsQuery = useQuery({
        ...
        queryFn: () => instance.get(`api/v1/members/me/hotels?page=${page - 1}`, {
            ...axios.defaults,
            useAuth: true,
        }).then((res) => res.data),
    ...
    const loadingState = hotelsQuery.isLoading || hotelsQuery.data?.objData.length === 0 ? "loading" : "idle";
                        ...
                        <TableRow key={item?.name}>
                            {
                                (columnKey) => {
                                    if (columnKey === "nickname") {
                                        return (
                                            <TableCell>
                                                <Link href={`/hotel/${item.id}`}>{getKeyValue(item, columnKey)}</Link>
                                            </TableCell>
                                        )
