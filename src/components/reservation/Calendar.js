...
export default function CalendarCustom({startDate, endDate, setStartDate, setEndDate}) {
	...
	const handleClickYear = (year) => {
		const startYears = new Date(`${getMonth(new Date()) + 1}-${getDate(new Date())}-${getYear(new Date()) - year}`);
		setStartDate(startYears);
	};

	const handleClickMonth = (month) => {
		const startYears = new Date(`${getMonth(new Date()) + 1 - month}-${getDate(new Date())}-${getYear(new Date())}`);
		setStartDate(startYears);
	};
